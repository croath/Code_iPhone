...
- (void) show:(iToastType) type{
	...
	label.shadowOffset = CGSizeZero; //CGSizeMake(1, 1);
	
	UIButton *v = [UIButton buttonWithType:UIButtonTypeCustom];
	if (image) {
#define PADDING 50
		v.frame = CGRectMake(0, 0, image.size.width + textSize.width + 80, MAX(textSize.height, image.size.height) + PADDING);
		label.center = CGPointMake(image.size.width + PADDING + (v.frame.size.width - image.size.width - PADDING) / 2, v.frame.size.height / 2);
	} else {
		v.frame = CGRectMake(0, 0, textSize.width + PADDING, textSize.height + PADDING);
